...
if __name__ == "__main__":
    ...
    L1 = 10  # length of pendulum 1 in m
    L2 = 52 # length of pendulum 2 in m
    ...
    for i in range(0, Num):
        ...
        th2t[i] = 2 * arctan(double(tt))

        th3[i] = arctan(double((-r2 * sin(th2t[i]) / (r1[i] - r2 * cos(th2t[i])))))
        #velocity equations
   ...
   # print(y2)
    ...
    # print(y2)
    fig = plt.figure(1)
    ax = fig.add_subplot(111, autoscale_on=False, xlim=(-10,80), ylim=(-1,20))
    ax.set_aspect('equal')
    ax.grid()
